...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "AI Coding Agents - Plan and implement" " code using Claude"
        ),
...
Examples:
  ...
  code-agent --dir ./project "Refactor module"
    ...
    parser.add_argument("task", nargs="?", help="The coding task to complete")
    parser.add_argument(
        "--issue", "-I", help="GitHub issue URL to research and " "implement"
    )
    parser.add_argument(
        "--dir", "-d", default=".", help="Working directory (default: .)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--json-events", action="store_true", help="Output events as JSON lines"
    )
    ...
    if args.interactive:
        ...
        issue_info = None
        if args.issue:
            issue_info = parse_github_issue_url(args.issue)
            if not issue_info:
                print("Error: Invalid GitHub " f"issue URL: {args.issue}")
                print(
...
def run_interactive(
    ...
    print("Enter coding tasks " "(Ctrl+D or 'exit' to quit)\n")
    ...
    while True:
        try:
            ...
            result = asyncio.run(run_coding_task(task, working_dir, verbose))
