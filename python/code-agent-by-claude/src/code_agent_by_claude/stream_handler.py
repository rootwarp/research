...
class DefaultStreamRenderer:
    ...
    def create_handler(self) -> StreamHandler:
        """Create a StreamHandler wired to this renderer."""
        handler = StreamHandler()
        if self.json_events:
            handler.on_all(self._handle_json)
        else:
            handler.on(
                EventType.PHASE_START, self._handle_phase
            )
            handler.on(
                EventType.PHASE_END, self._handle_phase
            )
            handler.on(
                EventType.TEXT_DELTA, self._handle_text
            )
            handler.on(
                EventType.TOOL_START, self._handle_tool
            )
            handler.on(
                EventType.TOOL_RESULT, self._handle_tool
            )
            handler.on(
